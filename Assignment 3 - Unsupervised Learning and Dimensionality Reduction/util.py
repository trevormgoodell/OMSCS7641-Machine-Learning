...
def testClassifier(X, y, classifier, dataset, learner, model_type, n_splits=5, subdir=""):
    avg_train_acc_list = []
    avg_test_acc_list = []
    ...
    conf_mat_title = "./images/" + dataset + "/" + learner + "/" + subdir + model_type + "_" + "confusion_matrix.txt"
    with open(conf_mat_title, "w") as output_file:
        output_file.write(str(final_conf_mat))

    class_rep_title = "./images/" + dataset + "/" + learner + "/" + subdir + model_type + "_" + "classification_report.txt"
...
def plotResults(results, title, dataset, learner, xlabel="Number of Training Instances Used", subdir = ""):
    ...
    fig_title = "./images/" + dataset + "/" + learner + "/" + subdir + title.replace(" ", "_")
